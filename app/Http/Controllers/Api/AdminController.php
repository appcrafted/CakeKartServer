...
class AdminController extends Controller
{
  ...
  public function orderdetails(Request $request)
  {
    ...
    if($request->user_id == 1){
        ...
        $checkorder=Order::select('order_from','full_name','email','mobile')
        ->where('order.id',$request['order_id'])
        ->first();

        if ($checkorder->order_from == "pos") {
            $status=Order::select('order.driver_id','order.user_id','order.address','order.building','order.landmark','order.pincode','order.promocode','order.discount_amount','order.order_number','order.status','order.order_notes','order.order_type','order.tax','order.lat','order.lang','order.delivery_charge','order.tax_amount')
            ->where('order.id',$request['order_id'])
            ->get()->first();
        } else {
        ...
        if($checkorder->order_from == "pos") {
            $username = $checkorder->full_name;
            $userprofile_image = url('/storage/app/public/images/profile/unknown.png');
            $usermobile = $checkorder->mobile;
            $useremail = $checkorder->email;
        } else {
            if (@$getusers["name"] == "") {
                $username = "";
                $userprofile_image = "";
        ...
        if(!empty($cartdata))
        {
            return response()->json(['status'=>1,'message'=>'success','delivery_address'=>@$status->address,'landmark'=>@$status->landmark,'building'=>@$status->building,'pincode'=>@$status->pincode,'order_type'=>@$status->order_type,'order_number'=>@$status->order_number,'name'=>@$username,'profile_image'=>@$userprofile_image,'mobile'=>@$usermobile,'lat'=>@$status->lat,'lang'=>@$status->lang,'data'=>@$cdata,'summery'=>$summery],200);
        }
        else
        {
